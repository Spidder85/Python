...
class Ray:
    def __init__(self, angle, player, map):
        ...
        self.is_fasing_down = self.rayAngle > 0 and self.rayAngle < math.pi
        self.is_fasing_up = not self.is_fasing_down
        self.is_fasing_right = self.rayAngle < 0.5 * math.pi or self.rayAngle > 1.5 * math.pi
        self.is_fasing_left = not self.is_fasing_right
